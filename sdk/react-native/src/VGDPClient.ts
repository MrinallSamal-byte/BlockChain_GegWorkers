...
function solidityProofHash(proof: { a: [string, string]; b: [[string, string], [string, string]]; c: [string, string] }): string {
...
export class VGDPClient {
  ...
  async startTracking(order: VGDPDeliveryOrder, options: VGDPTrackingOptions = {}): Promise<void> {
    ...
    console.log(
      ...
      `Activation radius: ${activationRadiusMeters}m, polling every ${pollingIntervalSeconds}s`
    );
  }

  async confirmDelivered(orderId: string, options: VGDPConfirmOptions): Promise<VGDPProofResult> {
    const order = this.activeOrders.get(orderId);
    if (!order) throw new Error(`No active tracking for order ${orderId}`);

    const bundle = await this._buildProofBundle(order, options);
    ...
    if (!response.ok) {
      ...
      throw new Error(`VGDP proof submission failed: ${(err as { error?: string }).error}`);
    }

    this.activeOrders.delete(orderId);
    return response.json() as Promise<VGDPProofResult>;
  }
  ...
  private async _buildProofBundle(order: VGDPDeliveryOrder, options: VGDPConfirmOptions): Promise<object> {
    const { location, deliveredAtEpoch, photoPHash, photoSalt, bundleNonce } = options;
    ...
    const wasmPath = `${this.config.zkAssetsBasePath}/location_within_radius.wasm`;
    const zkeyPath = `${this.config.zkAssetsBasePath}/location_within_radius_final.zkey`;
    ...
    const didSignature = await this.config.signDigest(ethers.getBytes(bundleDigest));
